...
* 5 independent Beta variables, assigning scores to each finger (could be grasp type).
* K in [0, 1], the count of number of fingers pressed
* A simple logits for categorical for each finger (instead of Beta) miiight make more sense?

When generating, we emit the top grasp per timestep.
"""
...
class Model(model_mod.ModelBase):
    ...
    def _make_decoder(self):
        ...
        Implementation Options:
          1. We use a MLPObsDecoder if we are outputting to inferred firing rates.
          2. We use Beta parameters (see midi.py) to model our confidence in each pose
            - could possibly use count categorical for number of poses to decode
            - ie 5 and 2 instead of 6 and 1?

        Each beta parameter has 2? then also K has 2
        """
        params = util.make_mlp(
            self.hparams,
            self.hparams.obs_decoder_fc_hidden_layers + [5 * 2 + 2])
        def _split_params(inp):
            finger_params, count_param = tf.split(inp, [5 * 2, 2], axis=-1)
            return (finger_params, count_param)  # Note: returning a tuple.
        single_finger_decoder = codec_mod.BetaDecoder(
            positive_projection=util.positive_projection(self.hparams))
        fingers_decoder = codec_mod.BatchDecoder(
            single_finger_decoder, event_size=[5], name="fingers_decoder")
        count_decoder = codec_mod.CategoricalDecoder(name="count_decoder")
        full_decoder = codec_mod.GroupDecoder((fingers_decoder, count_decoder))
